...
const DEFAULTS={defaultHold:.7,defaultDuration:.4,springSamples:36,precision:4,loop:true};
...
function compileManifest(m,opt={}){validate(m);const cfg={...DEFAULTS,...(m.calibration||{}),...opt},states=orderStates(m),schedule=buildSchedule(states,m.transitions||[],cfg),defs=[],body=[],report={matchedLayers:0,fallbackLayers:0,gradientTracks:0,pathMorphs:0,warnings:[]};for(const t of tracks(states)){const x=renderTrack(t,schedule,cfg,defs,report);if(x)body.push(x)}const first=states[0],w=finite(first.width,355),h=finite(first.height,240),d=schedule.totalDuration,svg=['<?xml version="1.0" encoding="UTF-8"?>',`<svg id="motion-svg" viewBox="0 0 ${fmt(w)} ${fmt(h)}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Figma calibrated SVG animation" data-duration="${fmt(d)}">`,'<defs>',...defs,'</defs>',`<g id="motion-scene">${body.join("")}</g>`,'</svg>',''].join('\n');return{svg,html:html(svg,d),report,schedule}}
