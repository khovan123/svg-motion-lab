...
// We will run exactly the matchGeometryGloballyV2 matching code for State 0
...
sortedContainers.forEach(layer => {
  ...
  if (layer.stableNodeId === '1:4181:@root/bar-chart[0]') {
    console.log("Debugging Bar Chart matching:");
    console.log("  Matched children count:", matchedChildEls.length);
    matchedChildEls.forEach((childEl, index) => {
      console.log(`  Child ${index} tag: <${childEl.tagName}> id="${childEl.id}" data-motion-id="${childEl.getAttribute('data-motion-id')}"`);
    });
    ...
    if (matchedChildEls.length > 0) {
      ...
      while (current && current.tagName && current.tagName.toLowerCase() !== 'svg') {
        console.log(`    Parent current tag: <${current.tagName}> matchedSet? ${matchedElementsSet.has(current)}`);
        if (current.tagName.toLowerCase() === 'g' && !matchedElementsSet.has(current)) {
          ...
          console.log(`      containsAll check: ${containsAll}`);
          if (containsAll) {
