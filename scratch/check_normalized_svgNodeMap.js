...
code = code.replace(
  ...
  `root.SvgMotionCompiler={validate,buildBaseSchedule:S.buildBaseSchedule,compile,matchGeometryGloballyV2,getAbsoluteBounds,canonicalizeManifest};`
);
...
console.log('svgNodeMap entries for state 0 containing mask-group or cyan:');
normalized.states[0].svgNodeMap.forEach(entry => {
  if (entry.stableNodeId.includes('mask-group') || entry.stableNodeId.includes('cyan')) {
    console.log('  ', entry.stableNodeId);
  }
});
