...
console.log('--- PATH MODES DIFFERENCES (Verify vs Patched) ---');
verifyTracks.forEach(vt => {
  const dt = parsedDistTracks.find(t => t.id === vt.id);
  if (dt) {
    if (vt.pathMode !== dt.pathMode) {
      console.log(`  [${vt.id}]: Verify = ${vt.pathMode}, Patched = ${dt.pathMode}`);
    }
  }
});

console.log('\n--- COLOR TRACKS DIFFERENCES (Verify vs Patched) ---');
verifyTracks.forEach(vt => {
  const dt = parsedDistTracks.find(t => t.id === vt.id);
  if (dt) {
    ...
    if (vHasColor !== dHasColor) {
      console.log(`  [${vt.id}]: Verify hasColor = ${vHasColor}, Patched hasColor = ${dHasColor}`);
    }
  }
});
