...
const correctedSvg = compiler.matchGeometryGloballyV2(state16);
const doc = new window.DOMParser().parseFromString(correctedSvg, 'image/svg+xml');

doc.querySelectorAll('*').forEach(el => {
  const mid = el.getAttribute('data-motion-id') || '';
  if (mid.includes('cyan')) {
    let chain = [];
    let curr = el;
    while (curr) {
      if (curr.tagName) {
        chain.push(curr.tagName + (curr.getAttribute('id') ? '#' + curr.getAttribute('id') : '') + (curr.getAttribute('data-motion-id') ? `[${curr.getAttribute('data-motion-id')}]` : ''));
      }
      curr = curr.parentNode;
    }
    console.log('Found cyan element:', chain.join(' -> '));
  }
});
