...
const state = normalized.states[3]; // State 3 (Property 1=4)
...
state.layers.forEach(layer => {
  ...
  if (el) {
    console.log(`  MATCHED: name="${layer.name}", id=${layer.stableNodeId} -> tag=${el.tagName.toLowerCase()} bounds=${JSON.stringify(layer.bounds)}`);
  } else {
    console.log(`  MISSING: name="${layer.name}", id=${layer.stableNodeId}`);
  }
});
