...
function matchGeometryGloballyV2(state) {
  ...
  // Group container background paths into a single <g> using data-motion-id
  const containerEls = [];
  rootSvg.querySelectorAll('[data-motion-id]').forEach(el => {
    const mid = el.getAttribute('data-motion-id');
    if (mid && mid.includes('@root/container') && el.tagName.toLowerCase() === 'path') {
      containerEls.push(el);
    }
  });

  console.log('containerEls length:', containerEls.length);
  ...
  sortedContainers.forEach(layer => {
    ...
    if (matchedGroup) {
      console.log('Matched container/rotor:', layer.stableNodeId, 'to tag:', matchedGroup.tagName);
...
function compile(manifest,options){
  ...
  return{svg,html,ir,report:{report:Object.assign({manifestSchema:manifest.schema,prototypeReady:true,snapshotsReady:true,infinite:schedule.infinite,customDuration:schedule.totalDuration},report),schedule},schedule,semanticReport:report}
}
