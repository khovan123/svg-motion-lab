...
console.log("Children of Bar Chart and their matched status:");
...
children.forEach(c => {
  const match = matchedNodes.get(c.stableNodeId);
  console.log(`- Layer: ${c.name} (${c.stableNodeId})`);
  console.log(`  Matched Element: ${match ? `<${match.tagName}> data-motion-id="${match.getAttribute('data-motion-id') || ''}"` : 'NONE'}`);
});
