...
const verifyIds = verifyTracks.map(t => t.id).sort();
const distIds = parsedDistTracks.map(t => t.id).sort();

console.log('Verify IDs count:', verifyIds.length);
console.log('Dist IDs count:', distIds.length);

console.log('\nIDs in Verify but not in Dist:');
verifyIds.forEach(id => {
  if (!distIds.includes(id)) console.log('  ', id);
});

console.log('\nIDs in Dist but not in Verify:');
distIds.forEach(id => {
  if (!verifyIds.includes(id)) console.log('  ', id);
});
