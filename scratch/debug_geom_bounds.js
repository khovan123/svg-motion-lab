...
// Copy getAbsoluteBounds from test_geom_matcher.js
...
console.log("\n--- Figma Layers (type frame/group) bounds in State 2 ---");
state.layers.forEach(l => {
  if (['FRAME', 'GROUP', 'COMPONENT', 'INSTANCE'].includes(l.type)) {
    console.log(`  Layer: "${l.name}", ID: "${l.stableNodeId}", Bounds: ${JSON.stringify(l.bounds)}`);
  }
});

console.log("\n--- SVG <g> tags and computed absolute bounds in State 2 ---");
const groups = rootSvg.querySelectorAll('g');
groups.forEach((g, idx) => {
  const b = getAbsoluteBounds(g);
  console.log(`  Group ${idx}: computed bounds: ${b ? JSON.stringify(b) : 'null'}, child tags: [${[...g.children].map(c => c.tagName).join(', ')}]`);
});
