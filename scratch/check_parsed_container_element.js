...
for (const scriptName of scripts) {
  ...
  if (scriptName === 'semantic-15.js') {
    fileCode = fs.readFileSync(path.join(scratchDir, 'semantic-15.js'), 'utf8');
  } else {
    fileCode = fs.readFileSync(path.join(webDir, scriptName), 'utf8');
  }
...
const ordered = baseSchedule.stateIds.map(id => norm.states.find(s => s.id === id));
const states = ordered.map(window.parseState);
const el = states[0].map.get('1:4181:@root/container[0]');
console.log('Element in states[0].map for container[0]:', el ? el.outerHTML : 'null');
