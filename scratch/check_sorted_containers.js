...
code = code.replace(
  ...
  `  console.log('containerLayers count:', containerLayers.length);
  containerLayers.forEach(l => console.log('  l:', l.stableNodeId, l.type));
  const sortedContainers = [...containerLayers].sort((a, b) => {`
);
fs.writeFileSync(path.join(scratchDir, 'semantic-15-debug-sorted.js'), code, 'utf8');
...
const scripts = [
  ...
  'semantic-11.js', 'semantic-12.js', 'semantic-15-debug-sorted.js', 'semantic-13.js', 'semantic-14.js',
  'semantic-16.js', 'semantic-runtime-fix.js'
];

for (const scriptName of scripts) {
  let fileCode;
  if (scriptName === 'semantic-15-debug-sorted.js') {
    fileCode = fs.readFileSync(path.join(scratchDir, 'semantic-15-debug-sorted.js'), 'utf8');
  } else {
