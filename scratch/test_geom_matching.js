...
// Initialize window/document for JSDOM
const dom = new JSDOM("<!DOCTYPE html><html><body></body></html>", { runScripts: "dangerously" });
const { window } = dom;
global.DOMParser = window.DOMParser;
global.XMLSerializer = window.XMLSerializer;

// Load semantic-15.js matching code or replicate it
const state = manifest.states[0];

// Let's run the exact matchGeometryGloballyV2 logic on state 0 and trace it
...
console.log("Matching trace for leaf layers:");
leafLayers.forEach((layer, lIdx) => {
  console.log(`\nLayer [${lIdx}]: Name="${layer.name}", StableId="${layer.stableNodeId}", Bounds=${JSON.stringify(layer.bounds)}, Type="${layer.type}"`);
  ...
  svgElements.forEach((el, elIdx) => {
    ...
    const isInside = bSVG.x >= bLayer.x - 0.5 && 
                     bSVG.y >= bLayer.y - 0.5 && 
                     bSVG.x + bSVG.width <= bLayer.x + bLayer.width + 0.5 && 
                     bSVG.y + bSVG.height <= bLayer.y + bLayer.height + 0.5;
    ...
    if (boundsMatch) {
      possible = true;
      reason = "boundsMatch";
    } else if (isInside && (layer.name.toLowerCase().includes('yellow') || layer.name.toLowerCase().includes('cyan') || layer.name.toLowerCase().includes('blue') || layer.name.toLowerCase().includes('orange') || layer.type === 'ELLIPSE')) {
      possible = true;
      reason = "isInside+name/type";
    }

    if (possible) {
      console.log(`    Possible match: SVG [${elIdx}] tag=${el.tagName.toLowerCase()}, bounds=${JSON.stringify(bSVG)}, diff=${diff.toFixed(2)}, reason=${reason}`);
      if (diff < minDiff) {
        minDiff = diff;
        bestMatch = el;
      }
  ...
  if (bestMatch) {
    ...
    console.log(`  ==> MATCHED to SVG element: tag=${bestMatch.tagName.toLowerCase()}, bounds=${JSON.stringify(getAbsoluteBounds(bestMatch))}`);
  } else {
