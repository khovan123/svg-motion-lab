...
console.log('Manifest mask-group_2[0]/cyan_2[0] bounds:');
const layer = state.layers.find(l => l.stableNodeId.includes('mask-group_2[0]/cyan_2[0]'));
console.log(layer ? layer.bounds : 'NOT FOUND');

console.log('\nAll paths in defs and their absolute bounds:');
if (defs) {
  defs.querySelectorAll('path').forEach(p => {
    console.log('Path d:', p.getAttribute('d').slice(0, 50));
    console.log('  getAbsoluteBounds:', compiler.getAbsoluteBounds(p));
  });
}
