...
function getAbsoluteBounds(el) {
  ...
  console.log(`Element transform: ${JSON.stringify(m)}`);
  const corners = [
    [local.x, local.y],
...
// Find path with M196 59
const rootSvg = doc.documentElement;
let el16 = null;
const collect = (node) => {
  if (node.tagName.toLowerCase() === 'path' && node.getAttribute('d')?.includes('M196 59')) {
    el16 = node;
  }
...
if (el16) {
  console.log(`Found el16, calculating bounds...`);
  const b = getAbsoluteBounds(el16);
  console.log(`Bounds: ${JSON.stringify(b)}`);
} else {
  console.log("el16 not found!");
}
