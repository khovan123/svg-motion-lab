...
// We want to check outputs.ir or outputs.tracks (mocked/hooked)
// Actually we can parse D from outputs.svg
...
distScripts.forEach(script => {
  ...
  if (match) {
    ...
    if (data.tracks) {
      ...
      console.log('Rotor track:', JSON.stringify(rotorTrack, null, 2));
      
      const childId = '1:4181:@root/container[0]/hugeiconsrefresh-03-stroke-rounded-1[0]/vector-stroke[0]';
      const childTrack = data.tracks.find(t => t.id === childId);
      console.log('Child track:', JSON.stringify(childTrack, null, 2));
    }
  }
});
