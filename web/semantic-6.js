...
S.buildSemanticSvg=function(manifest,schedule){
  ...
  const tracks=S.matchLayers(states),defs=[];
  const report={renderMode:'semantic',tracks:tracks.length,semanticTracks:0,pathMorphs:0,unsupportedTracks:0};
  let body='';
  for(const track of tracks){
    ...
    else if(S.trackSupported(track))markup=S.renderTrack(track,schedule.stateIds,schedule,defs,report);
    if(markup)body+=markup;else report.unsupportedTracks+=1;
  }
  const ratio=tracks.length?report.semanticTracks/tracks.length:0;
  if(report.semanticTracks<3||ratio<.3)return {svg:null,report};
  const svg='<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+S.round(first.width)+' '+S.round(first.height)+'" width="'+S.round(first.width)+'" height="'+S.round(first.height)+'" data-render-mode="semantic" data-duration="'+S.round(schedule.totalDuration)+'" data-infinite="'+schedule.infinite+'"><defs>'+defs.join('')+'</defs>'+body+'</svg>';
  return {svg,report};
};
...
S.buildSnapshotSvg=function(manifest,schedule,report){
  ...
  return '<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+S.round(first.width)+' '+S.round(first.height)+'" data-render-mode="snapshot">'+groups.join('')+'</svg>';
};
S.buildHtml=function(svg,schedule){
  ...
  const runtime="(()=>{const D="+data+",svg=document.querySelector('svg'),status=document.querySelector('#status');let start=performance.now(),paused=false,pauseAt=0;function tick(now){if(!paused){const elapsed=(now-start)/1000,t=D.infinite?elapsed%D.duration:Math.min(elapsed,D.duration);if(svg&&svg.setCurrentTime)svg.setCurrentTime(t);status.textContent=(D.infinite?'Infinite':'Play once')+' · '+t.toFixed(2)+' / '+D.duration.toFixed(2)+'s';if(D.infinite||elapsed<D.duration)requestAnimationFrame(tick)}else requestAnimationFrame(tick)}document.querySelector('#restart').onclick=()=>{start=performance.now();paused=false;requestAnimationFrame(tick)};document.querySelector('#pause').onclick=()=>{paused=!paused;if(paused)pauseAt=performance.now();else start+=performance.now()-pauseAt};if(svg&&svg.pauseAnimations)svg.pauseAnimations();requestAnimationFrame(tick)})()";
