...
S.buildBaseSchedule=manifest=>{
 const byId=new Map(manifest.states.map(s=>[s.id,s])),reactions=manifest.prototype.reactions||[],start=manifest.prototype.startStateId||manifest.startNodeId||manifest.states[0].id,stateIds=[],segments=[],seen=new Set();let current=start;
 while(byId.has(current)&&!seen.has(current)){
  ...
  segments.push({from:current,to:next,hold:Math.max(0,S.num(reaction.trigger.timeout,.7)),duration:Math.max(.001,S.num((action.transition||{}).duration,.4)),easing:(action.transition||{}).easing||{type:'EASE_OUT'},reactionId:reaction.id||null});current=next;
 }
